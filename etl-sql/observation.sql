...
;WITH hira_obs_codes_src AS (
  ...
  WHERE o.PTNTIDNO IS NOT NULL
    ...
    AND (
      EXISTS (SELECT 1 FROM hira_obs_codes_src hm WHERE hm.code_norm = UPPER(LTRIM(RTRIM(CAST(o.[청구코드] AS varchar(200))))))
      OR EXISTS (SELECT 1 FROM obs_code_meta_src pm WHERE pm.code_norm = UPPER(LTRIM(RTRIM(CAST(o.[청구코드] AS varchar(200))))))
    )
  UNION
  SELECT
    i.PTNTIDNO,
  ...
  WHERE i.PTNTIDNO IS NOT NULL
    ...
    AND (
      EXISTS (SELECT 1 FROM hira_obs_codes_src hm WHERE hm.code_norm = UPPER(LTRIM(RTRIM(CAST(i.[청구코드] AS varchar(200))))))
      OR EXISTS (SELECT 1 FROM obs_code_meta_src pm WHERE pm.code_norm = UPPER(LTRIM(RTRIM(CAST(i.[청구코드] AS varchar(200))))))
    )
), hira_obs_rows AS (
  SELECT DISTINCT
    UPPER(LTRIM(RTRIM(CAST(m.LOCAL_CD1 AS varchar(200))))) AS code_norm,
  ...
  FROM src_keys s
  LEFT JOIN hira_obs_rows hm ON hm.code_norm = s.claim_code_norm
)

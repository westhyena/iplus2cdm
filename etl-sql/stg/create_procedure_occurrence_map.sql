...
BEGIN
  CREATE TABLE [$(StagingSchema)].procedure_occurrence_map (
    ...
    CONSTRAINT PK_procedure_occurrence_map PRIMARY KEY (ptntidno, [date], [source], serial_no, order_no, map_index)
  );
END;
...
BEGIN
  ...
  IF NOT EXISTS (
    ...
    JOIN sys.tables t ON t.object_id = c.object_id
    JOIN sys.schemas s ON s.schema_id = t.schema_id
    WHERE s.name = '$(StagingSchema)' AND t.name = 'procedure_occurrence_map' AND c.name = 'map_index'
  )
  BEGIN
    ALTER TABLE [$(StagingSchema)].procedure_occurrence_map
      ADD map_index int NOT NULL CONSTRAINT DF_procedure_occurrence_map_mapindex DEFAULT (1) WITH VALUES;
  END;
  IF EXISTS (
  ...
  END;
  ALTER TABLE [$(StagingSchema)].procedure_occurrence_map
    ADD CONSTRAINT PK_procedure_occurrence_map PRIMARY KEY (ptntidno, [date], [source], serial_no, order_no, map_index);
END;
